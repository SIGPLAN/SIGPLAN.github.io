...
- [Objectives of OOPSLA](/Conferences/SPLASH/ObjectivesOfOOPSLA)

*Originally documented by Annabelle Satin (SIGPLAN PCO in the 2010's) and then amended during and after COVID by the SPLASH General Chairs, including [Alex Potanin](https://potanin.github.io/) (2022), [Manu Sridharan](https://manu.sridharan.net/) (2024), and [Ilya Sergey](https://ilyasergey.net/) (2025). Future General Chairs are expected to keep these notes up to date.*

**Important:** All documents related to SPLASH must be stored on the [SPLASH Google Drive](https://drive.google.com/drive/folders/0APXWu4PPnVzBUk9PVA) provided by SIGPLAN, which is the central repository for all essential documents. This ensures continuity across years and that future chairs can access prior materials.

The current SIGPLAN PCO (Professional Conference Organiser) is [Neringa Young](https://conf.researchr.org/profile/conf/neringayoung), who supports the GC across budgeting, logistics, and communications throughout the conference cycle.
...
ACM's [conference planning guide](https://www.acm.org/special-interest-groups/volunteer-resources/conference-planning) is the authoritative reference for running an ACM conference. It is worth at least scanning the [table of contents](https://www.acm.org/special-interest-groups/volunteer-resources/conference-planning/table-of-contents) to know which topics it covers (reimbursing speakers, taxes, etc.) so you can refer back later.

## What is SPLASH?

[SPLASH](https://sigplan.org/Conferences/SPLASH/) typically consists of the following events.
...
Co-located workshops must adhere to the [SIGPLAN Workshop Guidelines](https://www.sigplan.org/Resources/Guidelines/Workshops/). In particular, any proceedings must be published in the ACM Digital Library (or not at all).
...
See the [SPLASH 2022 Organizing Committee](https://2022.splashcon.org/committee/splash-2022-organizing-committee) as an example.
...
Various `@splashcon.org` email aliases must be updated every year. Ask the previous General Chair for the CSV file listing all the `splashcon.org` email aliases. Update the addresses for the current year, then email [technicalsupport@acm.org](mailto:technicalsupport@acm.org) and [ishelpdesk@hq.acm.org](mailto:ishelpdesk@hq.acm.org), attaching the updated CSV, and ask them to update the aliases.
...
- [SIGPLAN John Vlissides Award](/Awards/Vlissides) from the Doctoral Symposium
- OOPSLA Distinguished Papers and Reviewers
- OOPSLA Distinguished Artifacts and Artifact Reviewers
- [OOPSLA Most Influential Paper](/Awards/Conferences/OOPSLA) from 10 years prior
...
See also the [SIGPLAN Video Recording Policy](/Resources/Policies/VideoRecording).
...
The primary OOPSLA-specific guidance lives in [Objectives of OOPSLA](/Conferences/SPLASH/ObjectivesOfOOPSLA) and the [SIGPLAN PC Chair Resources](/Resources/Guidelines/ProChair). The notes below cover items that surface at the GC level.
...
Note: OOPSLA has operated a two-round model with different dates in recent years. See [Objectives of OOPSLA](/Conferences/SPLASH/ObjectivesOfOOPSLA) for the current scheduling guidance.

### PC / RC Composition

The PC (now RC) is selected by the PC/RC Chairs, subject to the [SIGPLAN Diversity Policy](/Resources/Policies/Diversity). Composition must be approved by the EC Vice-Chair **before** invitations are issued. The PC Chair should also work closely with the SPLASH SC Chair to avoid surprises.
...
Historically, OOPSLA's ERC was primarily used to review submissions from members of the Program Committee. This had several advantages: fewer conflicts of interest during the PC meeting, less imbalance in PC-meeting attendance, and consistent treatment for reviews of PC papers. Historically, PC papers received 4 reviews and non-PC papers received 3. ERC composition followed the same policies as the PC. See [Objectives of OOPSLA](/Conferences/SPLASH/ObjectivesOfOOPSLA) for the current model.
